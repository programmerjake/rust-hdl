...
pub use rust_hdl_macros::{PlainIO, IO};
...
impl<'ctx> Context<'ctx> {
    ...
    pub fn external_input<T: FixedTypeValue<'ctx>>(&'ctx self) -> Input<'ctx, T> {
        Input::external(self)
    }
    ...
    pub fn external_output<T: FixedTypeValue<'ctx>>(&'ctx self) -> Output<'ctx, T> {
